...
namespace SharpTrader
{
    public class BackTester
    {
        ...
        public void Start()
        {
            ...
            // log trading stats to be compared with those generated by ML training scripts
            TradingStats stats = new TradingStats();
            foreach (var op in this.Algo.ClosedOperations)
            {
                stats.AddOperation(op);
            }
            Logger.ForContext("Config", Config, true)
                .Information("ML Trading Stats: {Stats}", new { stats.Count, stats.Gain, stats.GainOverMaxDrowDown, stats.GainPerOperation, stats.SquareGainOverStdDev });
        }
